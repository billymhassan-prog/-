...
export default function AnalysisSidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const fetchAiInsights = useCallback(async () => {
    ...
    const repData = REPS.map(name => {
      ...
      return {
        name,
        attainment: att ? { pctToQuota: att.pctToQuota, currentPts: att.currentPts, quota: att.quota, ptsPerWk: att.currentPtsPerWk, status: att.status, wishlistPct: att.wishlistPct } : null,
        ...
        ndg: ndg ? { ndgPct: ndg.ndgPct, rfo: ndg.rfo, ads: ndg.ads } : null,
        ...
        coaching: coach ? { primaryArea: coach.primaryArea, urgency: coach.urgency } : null,
      };
    });

    const teamContext = {
      teamNDGPct: teamNDG.currentNDGPct,
      pctQElapsed: teamNDG.pctQElapsed,
      daysLeft: teamNDG.totalDaysInQ - teamNDG.daysElapsed,
    };
    ...
    try {
      ...
      const resp = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/coaching-insights`, {
        ...
        body: JSON.stringify({ repData, teamContext }),
        signal: controller.signal,
      });

      if (!resp.ok) {
        const errData = await resp.json().catch(() => ({ error: 'Request failed' }));
        setAiError(errData.error || `Error ${resp.status}`);
        setAiLoading(false);
        return;
      }

      const reader = resp.body?.getReader();
      if (!reader) { setAiError('No response stream'); setAiLoading(false); return; }
