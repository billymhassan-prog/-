...
export default function CoachingPage({ selectedRep }: { selectedRep: RepName | 'all' }) {
  ...
  const fetchRepAiInsight = useCallback(async (repName: RepName) => {
    ...
    const ndgData = repNDG.find(r => r.name === repName);
    ...
    const repData = {
      name: repName,
      attainment: att ? { pctToQuota: att.pctToQuota, currentPts: att.currentPts, quota: att.quota, ptsPerWk: att.currentPtsPerWk, wishlistPct: att.wishlistPct, status: att.status } : null,
      activity: act ? { calls: act.totalCalls, talkTime: act.totalTalkTime, emails: act.totalEmails, touchpoints: act.totalTouchpoints } : null,
      ndg: ndgData ? { ndgPct: ndgData.ndgPct, rfo: ndgData.rfo, ads: ndgData.ads } : null,
      pipeline: pipe ? { totalOpen: pipe.totalOpen, createdLW: pipe.createdLW, staleOpps: pipe.outOfDate, cwToDate: pipe.cwToDate } : null,
      cwnft: cw ? { totalCW: cw.totalCW, cwnft: cw.cwnft, pctNFT: cw.pctNFT } : null,
      coaching: coach ? { primaryArea: coach.primaryArea, urgency: coach.urgency, insight: coach.insight } : null,
    };
    ...
    try {
      ...
      const resp = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/coaching-insights`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          teamContext: {
            focusOnRep: repName,
            mode: 'individual_1on1_prep',
            teamMember: teamMembers.find(m => m.name === repName) || null,
            teamRoster: teamMembers.map(m => ({
              name: m.name, status: m.status, start_date: m.start_date, role: m.role,
              tenureDays: Math.floor((Date.now() - new Date(m.start_date).getTime()) / (1000 * 60 * 60 * 24)),
            })),
          },
        }),
      ...
      if (!resp.ok) {
        ...
        setRepAiError(errData.error || `Error ${resp.status}`);
        setRepAiLoading(false);
        return;
      }

      const reader = resp.body?.getReader();
      if (!reader) { setRepAiError('No response stream'); setRepAiLoading(false); return; }
      ...
      while (true) {
        ...
        while ((idx = buffer.indexOf('\n')) !== -1) {
          ...
          try {
            ...
            if (content) { accumulated += content; setRepAiInsight(accumulated); }
          } catch { /* partial */ }
        }
      }
      setRepAiLoading(false);
    } catch (e: any) {
      if (e.name !== 'AbortError') setRepAiError(e.message || 'Failed to get insights');
      setRepAiLoading(false);
    }
  }, []);
